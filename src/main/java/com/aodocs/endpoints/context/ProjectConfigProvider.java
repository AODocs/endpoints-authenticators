...
public class ProjectConfigProvider {
	...
	private final Supplier<ProjectsClient> projectsClientSupplier;
	private final Iam iamClient;
	private final String projectName;
	private final com.google.cloud.resourcemanager.v3.Project project;
	...
	public static void override(String applicationId, @Nullable Supplier<ProjectsClient> projectsClientSupplier, Iam iamClient) {
		INSTANCE = new ProjectConfigProvider(applicationId, projectsClientSupplier, iamClient);
	}

    private ProjectConfigProvider() {
        this(AppengineHelper.getApplicationId(), null, defaultIamClient(AppengineHelper.getApplicationId()));
    }
	...
	private ProjectConfigProvider(String applicationId, @Nullable Supplier<ProjectsClient> projectsClientSupplier, Iam iamClient) {
		this.projectsClientSupplier = projectsClientSupplier;
    ...
    @SneakyThrows(IOException.class)
	private ProjectsClient getProjectsClient() {
		return projectsClientSupplier != null ? projectsClientSupplier.get() : ProjectsClient.create();
	}
	...
	private ListServiceAccountsResponse listServiceAccounts() {
		...
		return iamClient.projects().serviceAccounts().list(projectName).execute();
	}
